...
const Front = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await axios.get("http://localhost:8001/api/content");
    ...
    {contents.length > 0 ? (
      contents.map((content, index) => (
        <div key={content._id || index} className="flex justify-center items-center">
          <div className="relative w-64 h-40 cursor-pointer dark:text-white">
            ...
            <div className="relative p-6 w-full h-full bg-white border-2 border-indigo-500 rounded-lg hover:scale-105 transition duration-500 flex flex-col justify-center">
              <div className="flex items-center">
                ...
                <h3 className="ml-3 text-lg font-bold text-gray-800">{content.day}</h3>
              </div>
              <p className="text-gray-600 text-sm mt-2">{content.title}</p>
            </div>
          </div>
        </div>
      ))
